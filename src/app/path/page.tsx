        ...
        {standings.map((s, i) => {
          ...
          return (
            <div
              ...
              `}
            >
              <span className={`w-4 shrink-0 text-right ${isTarget ? 'text-amber-300 font-bold' : 'text-slate-600'}`}>{i + 1}º</span>
              {s.team ? (
                <>
                  <TeamFlag team={s.team} size="sm" />
                  <span className={`flex-1 font-medium truncate ${isTarget ? 'text-white' : couldReach ? 'text-slate-300' : 'text-slate-500'}`}>
                    {s.team.name_en}
                  </span>
                </>
              ) : (
